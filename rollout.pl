################################################################################
#
# $URL: https://athena.redprairie.com/svn/prod/env/tags/2011.1.4/scripts/rollout.pl $
# $Revision: 234189 $
# $Author: mlange $
#
...
# OSSI ENHANCED
# *.csv compliant
#  Runs the *.csv from rollout directory.
################################################################################

################################################################################
# Updated 2015/06/05 by Vin Aggarwal
# OSSI ENHANCED
#  Runs the *sql from ddl directories.
#  Loads integrator export ($LESDIR/downloads/integrator/ALL)
################################################################################

################################################################################
# Updated 2016/02/25 by Vin Aggarwal
# OSSI ENHANCEMENTS
#  Skips sequences unless -S specified
#  Loads all views including those from SAL, DCS etc if any table/column DDL checked in
#  Behaviour and sequence of -D (load all DDL) flag changed
#  DB changed only in Node_1 of clustered env
################################################################################

################################################################################
# Updated 2019/01/10 by Ali Ahmed
#
# OSSI ENHANCEMENTS
#  Skips locales only when -L is specified
################################################################################
...
#OSSI global variables
my ($uc_cmd_line, $uc_change_db, $uc_slimp_file, $uc_ddl_path, $uc_ddl_executed, $uc_seq_executed, $uc_reload_views);
...
sub CheckRequirements
{
    ...
    #if (($opt_D && $uc_change_db)
    if (! -d $uc_ddl_path)
    {
      print "ERROR: DDL directory does not exist.\n";
      print "Path: $uc_ddl_path\n";
      return 1;
    }
...
sub CreateDir
{
    ...
    {
      print LOGFILE "\n";
      print LOGFILE "INFO: Directory already exists.\n";
      #SetExitStatus(2);
      return;
    }
...
sub ImportSLData
{
    ...
    print LOGFILE "Import File: $uc_slimp_file\n";
    ...
    if ( "$uc_slimp_file" )
    {
        print LOGFILE "Concatenate $pathname to $uc_slimp_file\n";
        open ( UC_SLIMP_HANDLE, ">>",  $uc_slimp_file ) or die "Could not open file $uc_slimp_file: $!";
        open ( UC_SLEXP_HANDLE, "<",   $pathname      ) or die "Could not open file $pathname: $!";

        while ( my $line = <UC_SLEXP_HANDLE> )
        {
            print UC_SLIMP_HANDLE $line;
        }
        close ( UC_SLIMP_HANDLE );
        close ( UC_SLEXP_HANDLE );
    }
    else
    {
        # Close the logfile.
...
sub LoadData
{
    ...
    #########START Added by Haris on 05/13/2010 ##################
    #########if datafile path is available then use the path from there instead of taking the path from ctl ##################
    if (dirname("$dataFile") ne ".")
    {
      $dataDir = ExpandEnvVars("%SOURCEDIR%/").dirname("$dataFile");
    }
    else
    {
      $dataDir = "$ctlFile";
    ...
    print LOGFILE "Data Dir    : $dataDir\n";
    ...
    # Make sure the data file exists.
    if ((! -r "$dbDir/$dataDir/$dataFile") && ($dataFile ne "*.csv"))
    {
      print LOGFILE "\n";
      print LOGFILE "ERROR: Data file does not exist.\n";
      SetExitStatus(1);
    ...
    {
      $command = "mload -H -c $ctlFile -D $dataDir -d $dataFile";
    }

    print LOGFILE "mload cmd   : $command\n";
...
sub Remove
{
    ...
    {
      print LOGFILE "\n";
      print LOGFILE "INFO: File does not exist.\n";
      #SetExitStatus(2);
      return;
    }
...
sub Replace
{
    ...
    {
      print LOGFILE "\n";
      print LOGFILE "INFO: Directory does not already exist.\n";
      #SetExitStatus(2);
      CreateDir ("$target");
      if (! -d "$target")
      {
    ...
    {
      print LOGFILE "\n";
      print LOGFILE "INFO: File does not already exist.\n";
      #SetExitStatus(2);
    }
    else
    {
...
# ------------------------------------------------------------------------------
#
# FUNCTION: RunOSSIInstallDDLDir
#
# PURPOSE:  Run all SQL scripts using the InstallSql.pl script.
#
# ------------------------------------------------------------------------------
sub RunOSSIInstallDDLDir
{
    my ($dir, $filetypes) = @_;
    my $status = 0;
    my $uc_ddl_directory = "$uc_ddl_path/$dir";

    # Don't bother if the ddl subdirectory doesn't exist.
    if (! -d $uc_ddl_directory)
    {
      return;
    }

    # or if it is empty
    opendir DIR, $uc_ddl_directory;
    ...
    # Change to the ddl subdirectory.
    if (! chdir($uc_ddl_directory))
    {
      print LOGFILE "\n";
      print LOGFILE "ERROR: Could not change to directory: $uc_ddl_directory.\n";
      print LOGFILE "       $!\n";
      SetExitStatus(1);
      return;
    }
    ...
    print LOGFILE "\nInstalling $dir from $uc_ddl_directory.\n";
    $status = InstallSqlExec($filetypes);

    # Check the status from the system call.
    if ($status != 0)
    {
      print LOGFILE "\n";
      print LOGFILE "Some errors in \"$dir\" scripts.\n";
    }
...
# ------------------------------------------------------------------------------
#
# FUNCTION: RunOSSIInstallAllSql
#
# PURPOSE:  Run all SQL scripts using the InstallSql.pl script.
#
# ------------------------------------------------------------------------------
sub RunOSSIInstallAllSql
{
  my $status = 0;

  print LOGFILE "Installing ALL custom DDL scripts ...\n\n";

  RunOSSIInstallDDLDir("Tables", "*.*");
  RunOSSIInstallDDLDir("Functions", "*.*");
  ReloadOSSIAllViews();
  RunOSSIInstallDDLDir("Triggers", "*.*");
  RunOSSIInstallDDLDir("Indexes", "*.*");
  RunOSSIInstallDDLDir("Packages", "*.*");
  #Not "Sequences",
}
...
sub RunOSSISQL
{
  ...
  # Set flag to load all views
  if (index($script,"$uc_ddl_path/Tables")==0)
  {
    $uc_reload_views = 1;
  }

  # check for Sequences
  if (index($script,"$uc_ddl_path/Sequences")==0)
  {
    if ($opt_S)
    {
      if ($opt_D)
      {
        if (! $uc_seq_executed)
        {
          ...
          RunOSSIInstallDDLDir('Sequences', "*.iesql");
          RunOSSIInstallDDLDir('Sequences', "*.seq");
          $uc_seq_executed = 1;
        }
        return;
      }
  ...
  if ($opt_D)
  {
    if (index($script,$uc_ddl_path)==0)
    {
      if (! $uc_ddl_executed) {
        RunOSSIInstallAllSql();
        $uc_ddl_executed = 1;
      }
      return;
    }
  ...
  if ($uc_ignore_err)
  {
    ...
    SetExitStatus(2); # we do not care
    return;
  }
  print LOGFILE "\n";
...
sub RunScript
{
    ...
    foreach $token (@command)
    {
        ...
        if ($token  =~ /UpdateSequences.pl/i)
        {
          $dbscript = 1;
        }
    }

    if ($dbscript && !$uc_change_db)
    {
      close(LOGFILE);
      open(LOGFILE, ">>$Logfile");
      LOGFILE->autoflush(1);
      print LOGFILE "\n";
      print LOGFILE "INFO: Skipping database change script: @command\n";
      return;
    }
...
# ------------------------------------------------------------------------------
# OSSI SCRIPT
# FUNCTION: RunOSSILoadExportedSLData
#
# PURPOSE:  This function loads SL data exported as downloads/integrator/ALL
#
# ------------------------------------------------------------------------------
sub RunOSSILoadExportedSLData
{
    my $uc_all_slimp_file = @_;

    my $status = 0;

    if (! -r $uc_all_slimp_file)
    {
        print "ERROR: Integrator export data does not exist.\n";
        print "File: $uc_all_slimp_file\n";
        exit 1;
    }
  ...
  UIprint("    * LOADING INTEGRATOR DATA FROM $uc_all_slimp_file.\n");
  print LOGFILE "\n===> Loading integrator data from $uc_all_slimp_file...\n\n";

  $command = "$SLDIR/bin/slImp -S -f $uc_all_slimp_file";

  # Execute the command.
  close (LOGFILE);
  UIprint("    * === Running $command\n");
  system("$command >>$Logfile 2>&1");
  #system("$command");
  if ($? != 0)
  {
  ...
  {
    print LOGFILE "\n===> Done Loading integrator export file\n";
  }
  if ($opt_X)
  {
    UIprint("    * DONE WITH INTEGRATOR DATA. status $status\n");
  }
    close (LOGFILE);
    return;
}
...
# ------------------------------------------------------------------------------
# OSSI SCRIPT
# FUNCTION: RunOSSIScriptToLoadSLData
#
# ------------------------------------------------------------------------------
sub RunOSSIScriptToLoadSLData
{
    my $status = 0;
    ...
    if (-r "$uc_slimp_file")
    {
        UIprint("    * LOADING INTEGRATOR DATA FROM $uc_slimp_file.\n");
        print LOGFILE "\n===> Loading integrator data from $uc_slimp_file...\n\n";
        $command = "$SLDIR/bin/slImp -v -f $uc_slimp_file";
        ...
        {
            print LOGFILE "\n===> Done Loading integrator - removing temp load file\n";
            unlink ( "$uc_slimp_file" );
    ...
    {
      print LOGFILE "\n";
      print LOGFILE "$uc_slimp_file does not exist so no integrator data to load\n";
    }
...
sub RunOSSIScriptToFixMLSForAllLocales
{
    my $status = 0;
    my $stmt = "";

    # Write a message to the logfile.
    UIprint("    * Entering OSSI FIX SCRIPT.\n");
    print LOGFILE "\n===> Running OSSI script to fix descriptions for all locale ids...\n\n";

    $stmt = "load ossi locale " .
            "    where default_all = '1' ";
    ...
    if ($status != 0)
    {
      UIprint("    * Failed OSSI FIX SCRIPT.\n");
      print LOGFILE "\n";
      print LOGFILE "ERROR: Could not run MSQL script successfully.\n";
      SetExitStatus(1);
      return;
    }
    UIprint("    * Successfully ran OSSI FIX SCRIPT.\n");
    return;
}
...
# ------------------------------------------------------------------------------
# OSSI SCRIPT
# FUNCTION: ReloadOSSIAllViews
#
# ------------------------------------------------------------------------------
sub ReloadOSSIAllViews
{
    foreach $layeredProduct (@PRODUCT_LIST)
    {
        if (ProductIsInstalled($layeredProduct))
        {
            ReloadViews($layeredProduct);
        }
    }
    RunOSSIInstallDDLDir('Views', "*.*");
}
...
sub ProcessHotFixScript
{
    ...
    while (<INFILE>)
    {
      ...
      {
        if ($command =~ /^dbupgrade$/i)               # Db Upgrade
        {
            DbUpgrade($line[0]);
        }
...
my $ossi_start_time = [Time::HiRes::gettimeofday()];
...
$uc_cmd_line = "@ARGV";
# Set the options
if (!getopts('hXDASL') || $opt_h || ($#ARGV!=0))
{
  print "\nUsage:perl -S rollout.pl <options> <rollout_file_name>\n";
  print "\nOptions:\n";
  print " -h: help (this message).\n";
  print " -X: do not load integrator changes.\n";
  print " -D: install all custom DDL instead of individual scripts. Excludes Sequences.\n";
  print " -A: load /downloads/integrator/ALL\n";
  print " -S: create Sequences. Use with -D for all sequences.\n";
  print " -L: do not load all Locales related data.";
  exit 1;
}
$HotFix = uc($ARGV[0]);
...
# OSSI-SA1
$uc_slimp_file = "$LESDIR/log/$HotFix-$datetime.slexp";
...
UIprint("    *** Start time is $datetime ***\n" );

# Open the logfile and set auto flushing.
open(LOGFILE, ">>$Logfile");
LOGFILE->autoflush(1);

print LOGFILE "Command line: $uc_cmd_line\n";

$uc_change_db = 1;
if ("$ENV{UC_CLUSTER_NODE}")
{
  UIprint("    * Cluster node detected.\n");
  if (!($ENV{UC_CLUSTER_NODE} =~ /^node_1$/i))
  {
    $uc_change_db = 0;
    UIprint("    * Skipping DB changes on clustered node.\n");
		print LOGFILE "Skipping DB changes on clustered node.\n";
  }
}
...
{
  # Reload all views
  if ($uc_reload_views && !$opt_D)
  {
    ReloadOSSIAllViews();
  }

  # Run user level OSSI script to fix the issues.
  UIprint("\n    * OSSI FIX SCRIPT process can take some time.\n");

  if ($opt_A)
  {
    RunOSSILoadExportedSLData("$LESDIR/downloads/integrator/ALL");
  }

  if (!$opt_X)
  {
    RunOSSIScriptToLoadSLData();
  }

  if (!$opt_L)
  {
  print "    * Loading locale related data. \n";
  RunOSSIScriptToFixMLSForAllLocales();
  }
}
# OSSI STuff ends
...
# OSSI ADDED
$datetime = Datetime( );
$ossi_elapsed = Time::HiRes::tv_interval($ossi_start_time);
UIprint("    *** End time is $datetime.  Elapsed: $ossi_elapsed seconds ***\n" );
# OSSI ENDED
